...
- [ ] Bump `typescript` to `~6.0.3` (D1).
- [ ] In `tsconfig.json`: `target: "ES2022"`, `lib: ["ES2022"]`, `module: "nodenext"`, `moduleResolution: "nodenext"`, plus `esModuleInterop: true`, `skipLibCheck: true`, `declarationMap: true`, `isolatedModules: true`. **Keep `sourceMap: true`** — D7 ships `src/`, so both map kinds resolve for consumers. Keep `strict` and the existing `noUnused*` flags. Remove the now-invalid `target: "es5"` and `moduleResolution: "node"`.
...
**Acceptance criteria:**
- `npx tsc --noEmit` exits 0 with no `TS5107` diagnostics. (Verified reachable: this exact option set compiles clean under 6.0.3 against the current source.)
- `npm run build` emits `dist/index.js`, `dist/index.d.ts`, and `dist/index.d.ts.map`.
